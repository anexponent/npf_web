    ...
    <section class="d-flex align-items-center page-hero  inner-page-hero " id="page-hero">
      ...
      <div class="container">
        <div class="hero-text-area centerd">
          <h1 class="hero-title  wow fadeInUp" data-wow-delay=".2s">Fallen Heros/Heroines</h1>
                ...
                <div class="col-12 ">
                  <div class="post-box">     <a class="post-link" href="post-single.html" title="How litespeed technology works to speed up your site ">
                    ...
                    <div class="post-summary">
                      <div class="post-info"><a class="info post-cat" href="{{route('hero.details',$item->id)}}"> <i class="bi bi-bookmark icon"></i>Fallen Hero/Heroine</a><a class="info post-author" href="#"> <i class=" bi bi-person icon"></i>Date Posted:{{$item->created_at}} </a></div>
                      <div class="post-text"><a class="post-link" href="{{route('hero.details',$item->id)}}">
                        <div class="tm-details"><a class="tm-link" href="{{route('hero.details',$item->id)}}">
                            <h6 class="tm-name">Officer's Number: {{$item->ap_force_number}}<br> Rank: {{$item->rank}} <br>Full Name: {{$item->first_name}} {{$item->last_name}}</h6></a><span class="tm-role text-danger">Died on:{{$item->date_occurrence}} </span></div>
                    </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="col-12">
                    <nav aria-label="Page navigation example">
                        ...
                        <ul class="pagination pagination-blog justify-content-center">
                            @if($allHero->total() > $allHero->perPage())
                            {{ $allHero->links() }}
                            @endif
