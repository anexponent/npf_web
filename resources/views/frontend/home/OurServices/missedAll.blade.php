    ...
    <section class="d-flex align-items-center page-hero  inner-page-hero " id="page-hero">
      ...
      <div class="container">
        <div class="hero-text-area centerd">
          <h1 class="hero-title  wow fadeInUp" data-wow-delay=".2s">Missing Persons</h1>
        ...
        <div class="row ">
          <div class="col-12 col-xl-8 ">
            <div class="posts-grid horizontal">
              <div class="row">
                @foreach($allMissed as $item)

                  <div class="col-12 ">
                  <div class="post-box">     <a class="post-link" href="{{url('/missing/details',$item->id)}}" title="How litespeed technology works to speed up your site "> 
                    ...
                    <div class="post-summary">
                      <div class="post-info"><a class="info post-cat" href="#"> <i class="bi bi-bookmark icon"></i>Missing</a><a class="info post-author" href="#"> <i class=" bi bi-person icon"></i>{{Carbon\Carbon::parse($item->created_at)->diffForhumans()}} </a></div>
                      <div class="post-text"><a class="post-link" href="{{url('/missing/details',$item->id)}}"> 
                          <h3 class="post-title"> <span class="tm-role text-danger">Missing Since:{{Carbon\Carbon::parse($item->created_at)->diffForhumans()}} <span> </h3> </a>

                          <p class="post-title"> Name: {{$item->first_name}} {{$item->last_name}} </p>
                          <div class="post-title"> Last Seen at: {!!$item->location_disappearance!!}  </div>
                                                  
                        <a class="read-more" href="{{url('/missing/details',$item->id)}}" title="How litespeed technology works to speed up your site ">read more<i class="bi bi-arrow-right icon "></i></a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="col-12">
                    <nav aria-label="Page navigation example">
                        ...
                        <ul class="pagination pagination-blog justify-content-center">
                            @if($allMissed->total() > $allMissed->perPage())
                            {{ $allMissed->links() }}
                            @endif
